...
**With Nodestradamus:** The dependency graph is built by `analyze_deps` and cached under `.nodestradamus/`. AI uses it by calling Nodestradamus MCP tools (`get_impact`, `analyze_graph`). When those tools are available, AI will:
